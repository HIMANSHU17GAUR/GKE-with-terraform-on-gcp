...
locals {
  host_network_ports = {
    gatekeeper = {
      controller_manager_port         = 8444
      controller_manager_metrics_port = 8888
      controller_manager_health_port  = 9090
    }
  }
}
...
provider "helm" {
  kubernetes {
    host                   = "https://${module.gke_enterprise.endpoint}"
    token                  = data.google_client_config.default.access_token
    cluster_ca_certificate = base64decode(module.gke_enterprise.ca_certificate)
  }
}

####################################
#-Create GKE Standerd Cluster-HIGA-#
####################################
...
module "gke_enterprise" {
  ...
  zones                                   = ["us-central1-a", "us-central1-b"] #Immutable
